...
public abstract class BaseRecyclerFragment<T, V extends BaseRecyclerViewAdapter<T>> extends BaseFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if (activity instanceof ToolBarActivity) {
			((ToolBarActivity) activity).getTbHelper().getToolBar().setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					UiUtil.toast("hahofdsf");
					mSwiperefreshRecyclerView.getRecyclerView().getLayoutManager().smoothScrollToPosition(mSwiperefreshRecyclerView.getRecyclerView(), null, 0);
				}
			});
		}*/
